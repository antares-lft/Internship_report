...
folder <- "/home/alafitte/Internship/Rapport de stage/Données/Population_size/Low_recombination/N500"
files <- file.path(folder, sprintf("resultats_final_%02d.csv", 1:20))
...
folder <- "/home/alafitte/Internship/Rapport de stage/Données/Population_size/Low_recombination/N1000"
files <- file.path(folder, sprintf("resultats_final_%02d.csv", 3:20))

# Lecture et traitement des données avec types explicites
combined_data <- files %>%
  map_dfr(~ read_csv(.x, col_types = cols(
    ...
    Nw_sum.1 = col_double(),
    Sw_sum.1 = col_double(),
    .default = col_guess()  # devine le reste
  )) %>%
    mutate(branch_length = .[[13]]) %>%  # 13e colonne comme branch_length
    select(species, dN, dS, Nw_sum.1, Sw_sum.1, branch_length)
  ) %>%
  filter(!is.na(dN), !is.na(dS), !is.na(Nw_sum.1), !is.na(Sw_sum.1), !is.na(branch_length)) %>%
  mutate(
    generations = branch_length / mu,
  ...
  filter(is.finite(dnds), is.finite(generations)) %>%
  mutate(species = fct_reorder(species, generations))
...
folder <- "/home/alafitte/Internship/Rapport de stage/Données/Population_size/Low_recombination/N2500"
files <- file.path(folder, sprintf("resultats_final_%02d.csv", 1:20))
...
folder <- "/home/alafitte/Internship/Rapport de stage/Données/Population_size/Low_recombination/N5000"
files <- file.path(folder, sprintf("resultats_final_%02d.csv", 1:20))
