...
ggplot() +
  ...
  annotate("text",
           x = max(donnees_comparees$log10_taille_pop),
           y = max(donnees_comparees$moyenne_points),
           label = paste0("Simulation: R² = ", r2_simu, ", slope = ", slope_simu),
           hjust = 1, size = 4.5, color = "black") +
  annotate("text",
           x = max(donnees_comparees$log10_taille_pop),
           y = max(donnees_comparees$moyenne_points) - 0.12,
           label = paste0("Theory: R² = ", r2_top5, ", slope = ", slope_top5),
           hjust = 1, size = 4.5, color = "red") +
...
# --- Sauvegarde ---
ggsave("/home/alafitte/Internship/Rapport de stage/Images/Results/Free_recombination/Report/pNpS_vs_pop_size.png",
...
ggsave("/home/alafitte/Internship/Rapport de stage/Images/Results/Free_recombination/Recent/pN_pS vs N estimated.png", width = 8, height = 6, dpi = 300)
...
# --- Fonction pour ajuster loess, prédire, et calculer Spearman ---
calc_loess_and_spearman <- function(data) {
  loess_fit <- loess(moyenne_dNdS ~ log10_taille_pop, data = data)
  data$loess_pred <- predict(loess_fit)
  
  spearman_rho <- cor(data$moyenne_dNdS, data$loess_pred, method = "spearman", use = "complete.obs")
  
  list(data = data, spearman_rho = spearman_rho)
}

# --- Calcul pour chaque source ---
simu_res <- calc_loess_and_spearman(filter(donnees_comparees, source == "simulation"))
top5_res <- calc_loess_and_spearman(filter(donnees_comparees, source == "top5"))

# --- Préparer données pour le plot ---
plot_data <- bind_rows(simu_res$data, top5_res$data)

# --- Limites pour annotations ---
y_max <- max(donnees_comparees$moyenne_dNdS, na.rm = TRUE)
x_max <- max(donnees_comparees$log10_taille_pop, na.rm = TRUE)

# --- Plot ---
ggplot() +
  # Points simulation
  geom_point(data = filter(plot_data, source == "simulation"),
             aes(x = log10_taille_pop, y = moyenne_dNdS, color = log10_branch_length),
             size = 3, shape = 16) +
  # Points top5
  geom_point(data = filter(plot_data, source == "top5"),
             aes(x = log10_taille_pop, y = moyenne_dNdS),
             size = 3.5, shape = 17, color = "red") +
  # Courbe loess simulation
  geom_line(data = filter(plot_data, source == "simulation"),
            aes(x = log10_taille_pop, y = loess_pred),
            color = "black", size = 1) +
  # Courbe loess top5
  geom_line(data = filter(plot_data, source == "top5"),
            aes(x = log10_taille_pop, y = loess_pred),
            color = "red", size = 1, linetype = "dashed") +
  scale_color_gradient(low = "blue", high = "yellow", guide = "none") +
  # Annotations Spearman rho
  annotate("text",
           x = x_max,
           y = y_max + 0.4,
           label = paste0("Simulation: Spearman R² = ", round(simu_res$spearman_rho, 3)),
           hjust = 1, size = 5, color = "black") +
  annotate("text",
           x = x_max,
           y = y_max + 0.3,
           label = paste0("Analytical prediction : Spearman R² = ", round(top5_res$spearman_rho, 3)),
...
# --- Sauvegarde ---
ggsave("/home/alafitte/Internship/Rapport de stage/Images/Results/Free_recombination/Report/dNdS_vs_pop_size.png",
       width = 8, height = 6, dpi = 300)
...
ggsave("/home/alafitte/Internship/Rapport de stage/Images/Results/Free_recombination/Recent/dN_dS vs N estimated.png", width = 8, height = 6, dpi = 300)
...
fichiers <- file.path(dossier, sprintf("resultats_final_%02d.csv", 1:50))

# Lire tous les fichiers et ajouter une colonne 'fichier' pour traçabilité
donnees_list <- fichiers %>%
  lapply(read_csv, col_types = cols()) %>%
  set_names(sprintf("fichier_%02d", 1:50))
...
files <- file.path(folder, sprintf("resultats_final_%02d.csv", 1:50))
...
donnees_par_espece <- combined_data %>%
  ...
  mutate(
    log10_branch_length = log10(moyenne_branch_length),
    log10_PinPis = log10(mean_point_PinPis),
    log10_dNdS = log10(mean_point_dNdS)
  ) %>%
  filter(
    is.finite(log10_PinPis),
    is.finite(log10_dNdS),
    is.finite(log10_branch_length)
  )

# === Régression type II : Major Axis sur log10 ===
modele <- lmodel2(log10_PinPis ~ log10_dNdS, data = donnees_par_espece)
...
# === Graphique ===
ggplot(donnees_par_espece, aes(x = log10_dNdS, y = log10_PinPis, color = log10_branch_length)) +
  geom_point(size = 3) +
  geom_abline(slope = slope, intercept = intercept, color = "black", linewidth = 1) +
  scale_color_viridis_c(option = "plasma", name = "log10(Branch length)") +
  annotate("text",
           x = max(donnees_par_espece$log10_dNdS),
           y = max(donnees_par_espece$log10_PinPis),
           label = paste0("R² = ", r2), hjust = 1, size = 4.5) +
  theme_minimal() +
  labs(
    title = "Régression MA : log10(pN/pS) ~ log10(dN/dS)",
    x = "log10(dN/dS)",
    y = "log10(pN/pS)"
  )
